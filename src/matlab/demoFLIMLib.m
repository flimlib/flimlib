...
fprintf('Basic mono-exponential fit...\n');
...
fprintf('2-exponential decay, 1-exponential fit...\n');
...
text(0.65, 0.4, '100 000', 'FontSize', 12);
text(0.65, 0.2, '1.50 ns', 'FontSize', 12);
...
text(0.02, 0.8, '\tau:', 'FontSize', 12);
text(0.27, 0.8, num2str(tau), 'FontSize', 12);
for i = 1 : numel(nrPhots)
    text(0.02, 0.8 - 0.2 * i, sprintf('\\tau_{%d}:', nrPhots(i)), ...
        'FontSize', 12);
...
text(time(start), yl(2) / 3, 'start', 'FontSize', 12, 'Rotation', 90, ...
    'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center')
plot(time((start + fit_start) * [1 1]), yl, 'k:', 'LineWidth', 2)
text(time(start + fit_start), yl(2) / 3, 'fit\_start', 'FontSize', 12, ...
     'Rotation', 90, 'VerticalAlignment', 'cap', ...
     'HorizontalAlignment', 'center')
plot(time((start + fit_end) * [1 1]), yl, 'k:', 'LineWidth', 2)
text(time(start + fit_end), yl(2) / 3, 'fit\_end', 'FontSize', 12, ...
     'Rotation', 90, 'VerticalAlignment', 'bottom', ...
     'HorizontalAlignment', 'center')
set(gca, 'YLim', yl, 'YTick', [], 'FontSize', 12)
%xlabel('Microtime [ns]')
%ylabel('Photon count')
title(tit)
...
if size(paramsLMA, 1) == 4
    text(0.02, 0.8, 'A:', 'FontSize', 12);
    text(0.02, 0.6, '\tau:', 'FontSize', 12);
    text(0.02, 0.4, 'Z:', 'FontSize', 12);
    text(0.02, 0.2, '\chi^2:', 'FontSize', 12);
    text(0.14, 0.8, num2str(round(paramsLMA(2))), 'FontSize', 12);
...
elseif size(paramsLMA, 1) == 6
    text(0.02, 0.88, 'A_1:', 'FontSize', 10);
    text(0.02, 0.72, '\tau_1:', 'FontSize', 10);
    text(0.02, 0.56, 'A_2:', 'FontSize', 10);
    text(0.02, 0.40, '\tau_2:', 'FontSize', 10);
    text(0.02, 0.25, 'Z:', 'FontSize', 10);
    text(0.02, 0.08, '\chi^2:', 'FontSize', 10);
    text(0.14, 0.88, num2str(round(paramsLMA(2))), 'FontSize', 10);
    text(0.14, 0.72, num2str(round(100 * paramsLMA(3)) / 100), ...
        'FontSize', 10);
    text(0.14, 0.56, num2str(round(paramsLMA(4))), 'FontSize', 10);
    text(0.14, 0.40, num2str(round(100 * paramsLMA(5)) / 100), ...
        'FontSize', 10);
    text(0.14, 0.25, num2str(round(100 * paramsLMA(1)) / 100), ...
        'FontSize', 10);
    text(0.14, 0.08, num2str(0 + round(100 * paramsLMA(6)) / 100), ...
        'FontSize', 10);
end
