...
def GCI_triple_integral_fitting_engine(period, photon_count, fit_start=0, fit_end=None,
    ...
    if instr is not None:
        ...
        # presumably shorter than photon_count
    ...
    # exclusive? the fitted and residuals had strange final values if included last index
    samples = fit_end-fit_start
    ...
    tries = _GCI_triple_integral_fitting_engine(
        ...
        _noise_types[noise_type], sig, Z, A, tau, np.ctypeslib.as_ctypes(fitted), 
        np.ctypeslib.as_ctypes(residuals), chisq, chisq_target)
