...
int Old_GCI_marquardt_compute_fn(float x[], float y[], int ndata,
								 ...
								 float alambda);
int Old_GCI_marquardt_compute_fn_instr(float xincr, float y[], int ndata,
...
int Old_GCI_marquardt_compute_fn_final(float x[], float y[], int ndata,
									   noise_type noise, float sig[],
									   float param[], int paramfree[], int nparam,
									   void (*fitfunc)(float, float [], float *, float [], int),
									   float yfit[], float dy[], float *chisq);
int Old_GCI_marquardt_compute_fn_final_instr(float xincr, float y[], int ndata,
											 int fit_start, int fit_end,
											 float instr[], int ninstr,
											 noise_type noise, float sig[],
											 float param[], int paramfree[], int nparam,
...
int Old_GCI_marquardt_step_instr(float xincr, float y[],
					...
					float **pfnvals, float ***pdy_dparam_pure, float ***pdy_dparam_conv,
					int *pfnvals_len, int *pdy_dparam_nparam_size);
int Old_GCI_marquardt_estimate_errors(float **alpha, int nparam, int mfit,
...
////////////////////////////TODO ARG COPIED FROM CORRESPONDING, EARLIER ECFUTIL.C:
...
int Old_GCI_gauss_jordan(float **a, int n, float *b)
{
...
void Old_GCI_covar_sort(float **covar, int nparam, int paramfree[], int mfit)
{
	int i, j, k;
	float temp;
...
#define JMAX 40
#define ACC 0.0002
static int Old_GCI_chisq(int nu, float chisq, float *root)
{
	int j;
								  ...
								  float d[], float **v, float interval)
{
	...
	switch ((int) (interval + 0.01)) {
		...
		default:
			ret = Old_GCI_chisq(mfit, interval/100, &chisq);
			if (ret) return -1;
			break;
	}
...
int Old_GCI_triple_integral(float xincr, float y[],
						int fit_start, int fit_end,
						noise_type noise, float sig[], 
...
int Old_GCI_triple_integral_instr(float xincr, float y[],
							  int fit_start, int fit_end,
							  float instr[], int ninstr,
							  noise_type noise, float sig[],
							  float *Z, float *A, float *tau,
							  ...
							  float *chisq, float chisq_target)
{
	...
	{
		Old_GCI_triple_integral(xincr, y, fit_start, fit_end, noise, sig,
...
//			division++;			 
	...
	{
		Old_GCI_triple_integral_instr(xincr, y, fit_start, fit_end, instr, ninstr, noise, sig,
								Z, A, tau, validFittedArray, residuals, &local_chisq, division);
...
int Old_GCI_marquardt(float x[], float y[], int ndata,
				  noise_type noise, float sig[],
				  float param[], int paramfree[], int nparam,
				  restrain_type restrain,
				  void (*fitfunc)(float, float [], float *, float [], int),
				  ...
				  float chisq_delta, float chisq_percent, float **erraxes)
{
	...
	alambda = -1;
	if (Old_GCI_marquardt_step(x, y, ndata, noise, sig,
	...
	for (;;) {
		...
		if (Old_GCI_marquardt_estimate_errors(alpha, nparam, mfit, evals,
...
int Old_GCI_marquardt_instr(float xincr, float y[],
					int ndata, int fit_start, int fit_end,
					float instr[], int ninstr,
					noise_type noise, float sig[],
					float param[], int paramfree[], int nparam,
					...
					float chisq_delta, float chisq_percent, float **erraxes)
{
	...
	alambda = -1;
	if (Old_GCI_marquardt_step_instr(xincr, y, ndata, fit_start, fit_end,
...
//	static float ochisq, paramtry[MAXFIT], beta[MAXFIT], dparam[MAXFIT];   // was static but now thread safe
	...
	if (*alambda < 0.0) {
		...
		if (Old_GCI_marquardt_compute_fn(x, y, ndata, noise, sig,
									 param, paramfree, nparam, fitfunc,
									 yfit, dy,
									 alpha, beta, chisq, *alambda) != 0)
			return -2;
	...
	if (Old_GCI_gauss_jordan(covar, mfit, dparam) != 0)
		return -1;

	/* Once converged, evaluate covariance matrix */
	if (*alambda == 0) {
		if (Old_GCI_marquardt_compute_fn_final(x, y, ndata, noise, sig,
										   param, paramfree, nparam, fitfunc,
										   yfit, dy, chisq) != 0)
			return -3;
		if (mfit < nparam) {  /* no need to do this otherwise */
			Old_GCI_covar_sort(covar, nparam, paramfree, mfit);
			Old_GCI_covar_sort(alpha, nparam, paramfree, mfit);
		}
		return 0;
	}
...
//	static float ochisq, paramtry[MAXFIT], beta[MAXFIT], dparam[MAXFIT];   // was static but now thread safe
	...
	if (*alambda < 0.0) {
		...
		if (Old_GCI_marquardt_compute_fn_instr(xincr, y, ndata, fit_start, fit_end,
										   instr, ninstr, noise, sig,
										   param, paramfree, nparam, fitfunc,
	...
	/* Matrix solution; GCI_gauss_jordan solves Ax=b rather than AX=B */
	if (Old_GCI_gauss_jordan(covar, *pmfit, dparam) != 0)
		return -1;

	/* Once converged, evaluate covariance matrix */
	if (*alambda == 0) {
		if (Old_GCI_marquardt_compute_fn_final_instr(
		...
		if (*pmfit < nparam) {  /* no need to do this otherwise */
			Old_GCI_covar_sort(covar, nparam, paramfree, *pmfit);
			Old_GCI_covar_sort(alpha, nparam, paramfree, *pmfit);
		}
		return 0;
	}
	...
	case NOISE_CONST:
		*chisq = 0.0;
		...
		for (i=0; i<ndata; i++) {
			...
			/* And find chi^2 */
			*chisq += dy[i] * dy[i];	
			//TODO ARG OK THESE AGREE NOW printf("dy_dparam is %f %f %f\n", dy_dparam[0], dy_dparam[1], dy_dparam[2]);
		}
					...
					int *pfnvals_len, int *pdy_dparam_nparam_size)
{
	...
	case NOISE_CONST:
		*chisq = 0.0;
		...
		for (i=fit_start; i<fit_end; i++) {
			yfit[i] += param[0];
			(*pdy_dparam_conv)[i][0] = 1;
			dy[i] = y[i] - yfit[i];
			for (j=0, l=0; l<nparam; l++) {
				if (paramfree[l]) {
					wt = (*pdy_dparam_conv)[i][l];
					for (k=0, m=0; m<=l; m++)
						if (paramfree[m])
							alpha[j][k++] += wt * (*pdy_dparam_conv)[i][m];
					beta[j] += wt * dy[i];
					j++;
				}
			}
		...
		for (i=fit_start; i<fit_end; i++) {
			...
			for (j=0, l=0; l<nparam; l++) {
				if (paramfree[l]) {
					...
					for (k=0, m=0; m<=l; m++)
						if (paramfree[m])
							alpha[j][k++] +=wt*(*pdy_dparam_conv)[i][m]*y_ymod; // was  wt * (*pdy_dparam_conv)[i][m];
					beta[j] += wt * dy[i];
					j++;
				}
			}
					   ...
					   float chisq_target, float chisq_delta, int chisq_percent)
{
	...
	ret = Old_GCI_marquardt_instr(xincr, trans, ndata, fit_start, fit_end,
...
/* Cleanup function */
void Old_GCI_marquardt_cleanup(void)
{
/*	if (fnvals_len) {
		free(fnvals);
