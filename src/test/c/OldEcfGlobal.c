...
int Old_GCI_alloc_global_matrix(global_matrix *m,
							int global, int local, int ntrans);
void Old_GCI_free_global_matrix(global_matrix *m);
void Old_GCI_copy_global_matrix(global_matrix dest, global_matrix src,
							int global, int local, int ntrans);
int Old_GCI_alloc_global_vector(global_vector *v,
							int global, int local, int ntrans);
void Old_GCI_free_global_vector(global_vector *v);
void Old_GCI_copy_global_vector(global_vector dest, global_vector src,
							int global, int local, int ntrans);
int Old_GCI_marquardt_global_exps_est_globals_instr(
...
int Old_GCI_marquardt_global_exps_est_params_instr(
			float xincr, float **trans, int ndata, int ntrans,
			int fit_start, int fit_end, float instr[], int ninstr,
			noise_type noise, float sig[], int ftype,
...
int Old_GCI_marquardt_global_exps_single_step(
				...
				float *alambda);
int Old_GCI_marquardt_global_compute_exps_fn(
			float xincr, float y[],
			int ndata, int fit_start, int fit_end,
			noise_type noise, float sig[], int ftype,
			float param[], int paramfree[], int nparam,
...
int Old_GCI_marquardt_global_compute_exps_fn_final(
			float xincr, float y[],
			int ndata, int fit_start, int fit_end,
			noise_type noise, float sig[], int ftype,
			float param[], int paramfree[], int nparam,
...
int Old_GCI_marquardt_global_exps_do_fit_instr(
		float xincr, float **trans, int ndata, int ntrans,
		int fit_start, int fit_end, float instr[], int ninstr,
		noise_type noise, float sig[], int ftype,
		float **param, int paramfree[], int nparam, restrain_type restrain, float chisq_delta,
		...
		int drop_bad_transients);
int Old_GCI_marquardt_global_exps_global_step(
				...
				int drop_bad_transients);
int Old_GCI_marquardt_global_compute_global_exps_fn(
		float xincr, float **trans, int ndata, int ntrans,
		int fit_start, int fit_end, float instr[], int ninstr,
		noise_type noise, float sig[], int ftype,
		float **param, int paramfree[], int nparam,
		...
		int drop_bad_transients);
int Old_GCI_marquardt_global_compute_global_exps_fn_final(
		float xincr, float **trans, int ndata, int ntrans,
		int fit_start, int fit_end, float instr[], int ninstr,
		noise_type noise, float sig[], int ftype,
		float **param, int paramfree[], int nparam,
...
int Old_GCI_marquardt_global_solve_eqn(global_matrix A, global_vector b,
			int mfit_global, int mfit_local, int ntrans);

int Old_GCI_marquardt_global_generic_do_fit_instr(
		float xincr, float **trans, int ndata, int ntrans,
		int fit_start, int fit_end, float instr[], int ninstr,
		...
		float **covar_scratch, float **alpha_scratch,
		float *chisq_trans, float *chisq_global);
int Old_GCI_marquardt_global_generic_global_step(
				float xincr, float **trans,
				int ndata, int ntrans, int fit_start, int fit_end,
				float instr[], int ninstr,
				noise_type noise, float sig[],
				...
				float *chisq_trans, float *chisq_global,
				float **alpha_scratch, float *alambda);
int Old_GCI_marquardt_global_compute_global_generic_fn(
		float xincr, float **trans, int ndata, int ntrans,
		int fit_start, int fit_end, float instr[], int ninstr,
		noise_type noise, float sig[],
		float **param, int paramfree[], int nparam, int gparam[],
...
int Old_GCI_marquardt_global_compute_global_generic_fn_final(
		float xincr, float **trans, int ndata, int ntrans,
		int fit_start, int fit_end, float instr[], int ninstr,
		noise_type noise, float sig[],
		float **param, int paramfree[], int nparam, int gparam[],
...
int Old_GCI_marquardt_global_exps_instr(float xincr, float **trans,
					int ndata, int ntrans, int fit_start, int fit_end,
					float instr[], int ninstr,
					noise_type noise, float sig[], int ftype,
					float **param, int paramfree[], int nparam,
...
//	printf("that took: %f secs.\n", Timer()-time); time=Timer();
	...
	ret = Old_GCI_marquardt_global_exps_est_globals_instr(
...
//	printf("that took: %f secs.\n", Timer()-time); time=Timer();
	...
	ret = Old_GCI_marquardt_global_exps_est_params_instr(
			xincr, trans, ndata, ntrans, fit_start, fit_end,
...
//	printf("that took: %f secs.\n", Timer()-time); time=Timer();
	...
	ret = Old_GCI_marquardt_global_exps_do_fit_instr(
			xincr, trans, ndata, ntrans, fit_start, fit_end,
			scaled_instr, ninstr, noise, sig, ftype,
			param, paramfree, nparam, restrain, chisq_delta,
			exp_pure, exp_conv, fitted, residuals, covar, alpha,
...
//										&Z, &A, &tau, NULL, NULL, NULL);
	...
	if (Old_GCI_marquardt_global_exps_calculate_exps_instr(
	...
	for (i=0; i<ntrans; i++) {
		ret = Old_GCI_marquardt_global_exps_do_fit_single(
		...
		float **covar, float **alpha, float *chisq)
{
	...
	alambda = -1;
	if (Old_GCI_marquardt_global_exps_single_step(
				...
				float *alambda)
{
	...
	if (*alambda < 0.0) {
		...
		if (Old_GCI_marquardt_global_compute_exps_fn(
	...
	if (*alambda == 0) {
		if (Old_GCI_marquardt_global_compute_exps_fn_final(
					xincr, y, ndata, fit_start, fit_end, noise, sig,
					ftype, param, paramfree, nparam, exp_conv,
					yfit, dy, chisq) != 0)
			return -4;
...
////					exp_conv, fitted[i], residuals[i],
	...
	/* We now begin our standard Marquardt loop, with several
	   modifications */
	alambda = -1;
	ret = Old_GCI_marquardt_global_exps_global_step(
				xincr, trans, ndata, ntrans, fit_start, fit_end,
	...
	if (ret != 0) {
		...
		if (ret != -1) {
			...
			alambda = 0.0;
			Old_GCI_marquardt_global_exps_global_step(
				xincr, trans, ndata, ntrans, fit_start, fit_end,
				instr, ninstr, noise, sig, ftype,
				param, paramfree, nparam, restrain, exp_pure, exp_conv,
				fitted, residuals, chisq_trans, chisq_global,
				...
				int drop_bad_transients)
{
	...
	if (*alambda < 0.0) {
		...
		if (initialised) {
			Old_GCI_free_global_matrix(&alpha);   Old_GCI_free_global_matrix(&covar);
			GCI_ecf_free_matrix(paramtry);        Old_GCI_free_global_vector(&beta);
			Old_GCI_free_global_vector(&dparam);  free(ochisq_trans);
			initialised = 0;
		}

		if (Old_GCI_alloc_global_matrix(&alpha, mfit_global, mfit_local, ntrans)
			!= 0)
			return -1;

		if (Old_GCI_alloc_global_matrix(&covar, mfit_global, mfit_local, ntrans)
			!= 0) {
			Old_GCI_free_global_matrix(&alpha);
			return -1;
		}
		...
		if (Old_GCI_alloc_global_vector(&beta, mfit_global, mfit_local, ntrans)
			!= 0) {
			Old_GCI_free_global_matrix(&alpha);  Old_GCI_free_global_matrix(&covar);
		...
		if (Old_GCI_alloc_global_vector(&dparam, mfit_global, mfit_local, ntrans)
			!= 0) {
			Old_GCI_free_global_matrix(&alpha);  Old_GCI_free_global_matrix(&covar);
			GCI_ecf_free_matrix(paramtry);       Old_GCI_free_global_vector(&beta);
			return -1;
		...
		if ((ochisq_trans = (float *) malloc(ntrans * sizeof(float)))
			...
			Old_GCI_free_global_matrix(&alpha);  Old_GCI_free_global_matrix(&covar);
			GCI_ecf_free_matrix(paramtry);       Old_GCI_free_global_vector(&beta);
			Old_GCI_free_global_vector(&dparam);
			return -1;
		}

		initialised = 1;

		if (Old_GCI_marquardt_global_compute_global_exps_fn(
	...
	if (*alambda == 0) {
		if (Old_GCI_marquardt_global_compute_global_exps_fn_final(
					xincr, trans, ndata, ntrans,
					fit_start, fit_end, instr, ninstr, noise, sig, ftype,
					param, paramfree, nparam, mfit_global, mfit_local,
	...
	/* Alter linearised fitting matrix by augmenting diagonal
	   elements. */
	Old_GCI_copy_global_matrix(covar, alpha, mfit_global, mfit_local, ntrans);
	Old_GCI_copy_global_vector(dparam, beta, mfit_global, mfit_local, ntrans);
	...
	/* Matrix solution; GCI_solve solves Ax=b rather than AX=B */
	if (Old_GCI_marquardt_global_solve_eqn(covar, dparam,
	...
	if (*chisq_global < ochisq_global) {
		*alambda *= 0.1;
		...
		for (i=0; i<ntrans; i++)
			ochisq_trans[i] = chisq_trans[i];
		Old_GCI_copy_global_matrix(alpha, covar, mfit_global, mfit_local, ntrans);
		Old_GCI_copy_global_vector(beta, dparam, mfit_global, mfit_local, ntrans);
		for (i=0; i<ntrans; i++) {
			for (j=0; j<nparam; j++)
		...
		int drop_bad_transients)
{
	...
	*chisq_global = 0.0;
	...
	for (i=0; i<ntrans; i++) {
		...
		ret = Old_GCI_marquardt_global_compute_exps_fn(
		...
		float *chisq_trans, float *chisq_global, int drop_bad_transients)
{
	...
	*chisq_global = 0.0;
	...
	for (i=0; i<ntrans; i++) {
		...
		/* This transient is fine! */
		ret = Old_GCI_marquardt_global_compute_exps_fn_final(
					xincr, trans[i], ndata, fit_start, fit_end, noise, sig,
					ftype, param[i], paramfree, nparam,
			...
			int mfit_global, int mfit_local, int ntrans)
{
	...
	if ((saved_global != mfit_global) || (saved_local != mfit_local) ||
		...
		if (saved_global > 0) {
			...
			Old_GCI_free_global_vector(&x);
			saved_global = 0;
		}
		if ((QS = GCI_ecf_matrix(mfit_global, mfit_local*ntrans)) == NULL)
			return -1;
		if (Old_GCI_alloc_global_vector(&x, mfit_global, mfit_local, ntrans)
			!= 0) {
			GCI_ecf_free_matrix(QS);
			return -1;
		}
	...
	Old_GCI_copy_global_vector(b, x, mfit_global, mfit_local, ntrans);
...
/* These functions are essentially the same as the above functions
   ...
   Old_GCI_marquardt_global_exps_do_fit_instr and the latter's dependents,
...
int Old_GCI_marquardt_global_generic_instr(float xincr, float **trans,
					int ndata, int ntrans, int fit_start, int fit_end,
					float instr[], int ninstr,
					noise_type noise, float sig[],
					float **param, int paramfree[], int nparam, int gparam[],
					...
					float chisq_trans[], float *chisq_global, int *df)
{
	...
	ret = Old_GCI_marquardt_global_generic_do_fit_instr(
			xincr, trans, ndata, ntrans, fit_start, fit_end,
			scaled_instr, ninstr, noise, sig,
		...
		float *chisq_trans, float *chisq_global)
{
	...
	/* We now begin our standard Marquardt loop, with several
	   modifications */
	alambda = -1;
	ret = Old_GCI_marquardt_global_generic_global_step(
				xincr, trans, ndata, ntrans, fit_start, fit_end,
				instr, ninstr, noise, sig,
	...
	if (ret != 0) {
		...
		if (ret != -1) {
			/* Wasn't a memory error, so unallocate arrays */
			alambda = 0.0;
			Old_GCI_marquardt_global_generic_global_step(
				xincr, trans, ndata, ntrans, fit_start, fit_end,
				instr, ninstr, noise, sig,
				param, paramfree, nparam, gparam, restrain, chisq_delta, fitfunc,
				fitted, residuals, chisq_trans, chisq_global,
				...
				float **alpha_scratch, float *alambda)
{
	...
	if (*alambda < 0.0) {
		...
		if (Old_GCI_marquardt_global_compute_global_generic_fn(
					xincr, trans, ndata, ntrans,
	...
	/* Once converged, evaluate covariance matrix */
	if (*alambda == 0) {
		if (Old_GCI_marquardt_global_compute_global_generic_fn_final(
					xincr, trans, ndata, ntrans,
					fit_start, fit_end, instr, ninstr, noise, sig,
					param, paramfree, nparam, gparam,
					mfit_global, mfit_local, gindex, lindex, fitfunc,
