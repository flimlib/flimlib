...
//TODO ARG kludge
int start_x = fit_start / 256;
fit_start = fit_start % 256;
...
t0 = (fit_start - start_x) * xincr;
	...
	*Z = (d1 - (*A) * (*tau) * exp_t0_tau * (1 - exp_dt_tau)) / dt;
	...
	//  NOISE_GAUSSIAN_FIT and NOISE_MLE not implemented for triple integral
...
{
    printf("ECFSIngle 366: GCI TRIPLE INTEGRAL FITTING ENGINE");
					...
					float chisq_percent, float **erraxes)
{
	...
	alambda = -1;
        int retnValue = GCI_marquardt_step_instr(xincr, y, ndata, fit_start, fit_end,
...
//			sig2i = 1.0/yfit[i];
			...
			for (j=0, l=0; l<nparam; l++) {
				if (paramfree[l]) {
					...
					beta[j] -= (1.0 - y_ymod)*wt; // was += wt * dy[i];
					j++;
				}
			}
			// And find chi^2
			if (yfit[i]<=0.0)
				*chisq += 2.0*dy[i];   // to avoid NaN from log, not strictly correct
			else if (y[i]==0.0)
				*chisq += 2.0*dy[i];   // to avoid NaN from log
			else
				*chisq += 2.0*dy[i] - 2.0*y[i]*log(yfit[i]/y[i]); // was dy[i] * dy[i] * sig2i;
		}
		break;
