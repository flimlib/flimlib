@echo off
if not exist flimlib-cmd.exe goto :file_error
echo Test 1 - Mono
call flimlib-cmd settings1.ini data.dat > console_output.txt
...
echo Test 2 - Bi
call flimlib-cmd settings2.ini data.dat > console_output.txt
...
echo Test 3 - Tri
call flimlib-cmd settings3.ini data.dat > console_output.txt
...
call flimlib-cmd settings4.ini data.dat > console_output.txt
...
echo Test 5 - Phasor
call flimlib-cmd settings5.ini data.dat > console_output.txt
...
:file_error
echo Could not find flimlib-cmd-exe, place it in this folder.
pause
goto :eof
